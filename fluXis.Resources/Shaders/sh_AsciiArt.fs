layout(std140, set = 0, binding = 0) uniform m_AsciiArtParameters
{
    vec2 g_TexSize;
    float g_Strength;
    float g_CellSize;
...
// https://www.shadertoy.com/view/lssGDj <-- modified from here
...
void main(void) {
    ...
    float cellSize = max(1.0, g_CellSize * 32.0);
	vec3 col = texture(sampler2D(m_Texture, m_Sampler), floor(pix/cellSize) * cellSize / g_TexSize.xy).rgb;
    ...
    vec2 p = mod(pix / (cellSize * 0.5), 2.0) - vec2(1.0);
    col = col * character(n, p);

    // test test test test test test
    vec4 original = texture(sampler2D(m_Texture, m_Sampler), gl_FragCoord.xy / g_TexSize);
    vec4 ascii = vec4(col, 1.0);
    o_Colour = mix(original, ascii, g_Strength);
}